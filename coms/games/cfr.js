...
IS.#addingInfosets is only checked for in IS.#put_priv. It is only set to true
in @EYURK in IS.init(). It is set to false by IS.readFromDisk() and  IS.stopAdding(), 
which is called only in Bluff_NS.initInfosets().

* * *

The "main funcs" are in CFR_NS and PCS_NS, which are the CLI interfaces into the library calls,

Both main functions:

if (argc < 2) {
	initInfosets();//Calls IS.dumpToDisk()
	exit(-1);
}

There is also technically a "main func" in BluffCounter_NS, but that is just a simple
utility to help with the creation of other types of bluff games (e.g. more dice, and 
using imperfect recall).

Bluff_NS.dumpInfosets() is the only other place where InfosetStore.dumpToDisk()
is called, but this is only called in currently commented out places in the
various main funcs (e.g. @WSUROK). I assume that these should be incommented in
"live" installations of the codebase. Otherwise, there doesn't seem to be
anywhere else that actual data (determined by the game algorithm) is written to
disk. Just before these commented out are calls to "report", which looks like:

outf << iter << " " << totaltime << " " << bound << " " << conv << " " << nodesTouched << endl;

So this just looks like a basic logging system to see how long the algorithm
takes, rather than doing everything that is needed to get everything working,
like writing files with meaningful stragety data in them.

* * *

readBytes is called only by readFromDisk, which is called only by the main funcs
(it is also called from importValues, but importValues isn't itself called anywhere, 
so it is commented out.)

So any infoset that is read during the runtime algorithm is read from memory.
...
void getInfoset(GameState & gs, int player, unsigned long long bidseq, Infoset & is, unsigned long long & infosetkey, int actionshere){
	...
	bool ret = gIss.get(infosetkey, is, actionshere, 0);
	assert(ret);
}
...
#get_priv(){
//Fields that are updated in the infoset
//...
...
if (arg && !Number.isFinite(arg)) DIE("Invalid arg given to 'new FVector()'");
...
//Otherwise, there are 4 copy assignments
  FVector() 
    : size(0) 
  { 
this.#elements = [];
...
multBy(arg){//«
	if (arg instanceof FVector){
		...
		if (this.#size != arg.length) DIE("Invalid arg dimension to SVector.multBy");
		for (let i=0; i < this.#size; i++){
			this.#elements[i]*=arg[i];
		}
	}
	...
	else DIE("Invalid arg to FVector.multBy");
}//»
addBy(arg){//«
	if (arg instanceof FVector){
		if (this.#size != arg.getSize()) DIE("Invalid arg dimension to FVector.addBy");
		for (let i=0; i < this.#size; i++){
			this.#elements[i]+=arg.getElem(i);
		}
	}
	else DIE("Invalid arg to FVector.addBy");
}//»
...
//This logic is unnecessary in a 1-dice game, for the reasons explained below
// i => p1roll-1, p2roll-1
...
//integer, such that a roll of 5, 3, 1 => 531
//For just 1 dice, a roll of 5 => 5
...
//So: roll[0] == 0 && roll[2] == 0 !?!
	}
/*Unused pathway with Bluff(1,1)
	else if (dice == 2) {
...
//int whowon(GameState & gs, int & delta){
	...
	return whowon6(gs.prevbid, bidder, gs.callingPlayer, gs.p1roll, gs.p2roll, delta);
}//»
const whowon1 = (gs) => {//«
...
/*
I guess when they are sorted, different permutations become the same key, e.g.
...
//		let rollcopy = rolls.slice();
		// now sort
//		bubsort(rollcopy, num_dice);
//Why would you want to sort in increasing order if nextRoll looks like it
//wants to increase the values from back to front (i.e. increasing order)?
//		rollcopy.sort((a,b)=>{if (a>b) return 1; if (b>a) return -1; return 0;})
...
//	bubsort(chanceOutcomes, numChanceOutcomes);
	chanceOutcomes.sort((a,b)=>{if (a>b) return 1; if (b>a) return -1; return 0;})
...
//	let filename = gFilepref + prefix + "." + to_string(gIter) + ".dat";//Was: string
	let filename = `${gFilepref}${prefix}.${gIter}.dat`;
...
// get the information set for update player where the chance outcome is replaced 
// by the specified one
	infosetkey = bidseq << gIscWidth; 
	infosetkey |= chanceOutcome; 
	infosetkey <<= 1; 
	if (player == 2) infosetkey |= 1; 

	let ret = false;//Was: bool

	ret = gIss.get(infosetkey, is, actionshere, 0); 
...
// This function should never be called by this code base because optimistic averaging is not included here.
	if (gIter > is.lastUpdate) {
		for (let a = 0; a < actionshere; a++) {//Was: int
			let inc =   (gIter - is.lastUpdate)//Was: dbl
				   * myreach
				   * is.curMoveProbs[a];
...
const computeBestResponses1 = (avgFix) => {//«
//double computeBestResponses(bool avgFix){
	let p1value = 0.0;//Was: dbl
	let p2value = 0.0;//Was: dbl
	return computeBestResponses3(avgFix, p1value, p2value);
}//»
...
//Now we are returning [b1, b2]
//	gIss.computeBound(b1, b2); 
/*If we are running the pcs algorithm, we are already doing this exact same thing
@WIGUPM. So we should either comment out this one or that one
*/
...
//	FVector<double> reach2(numChanceOutcomes(2), 1.0); 
	...
	cout.precision(15);
	log("gIter = " , gIter , " nodes = " , gNodesTouched , " conv = " , conv); 
...
//unsigned long long maxIters = 0; 
	...
	if (argc < 2) {
		...
		gIss.readFromDisk(filename);

		if (argc >= 3)
		maxIters = parseInt(argv[2]);
	}  
...
//gIter is defined in bluff.h: @ITER
	if (parts.size() != 3 || parts[1] == "initial") gIter = 1; 
	else gIter = parseInt(parts[1]); 
	log("Set iteration to " , gIter);
	gIter = MAX(1,gIter);
...
//This is defined in br.cpp and it calls expectimaxbr, which calls computeActionDist
			conv = computeBestResponses3(false, p1value, p2value);
...
//»
	...
	for (let i = 0; i < oppDice+1; i++) opp_probs[i] = 0.0;//Was: int
...
// Now, we need to construct a vector that contains a payoff per chance outcome of the updatePlayer
...
/*else if (gs.initRound && gs.curRound > 1){// Deal board cards here«
DOKUNX: If this is the time to deal (public) poker board cards (flop, turn, river),
...
//There is nothing like: phase++, etc.
	...
	if (phase == 2) {//«
		...
		if (gIter > 1 && updatePlayer == 2 && player == 2 && reach2.allEqualTo(0.0)) {
			result1.reset(0.0);
			result2.reset(0.0);
			return;
		}
...
//	P<n>CO = number of chance event outcomes (dice combinations), per player, at chance nodes: 
//	#dice	#outcomes
//  1		6
...
//3-player means alternate players in a 2-player game
	...
	if (player == updatePlayer && phase == 1) {//«
		...
		let moveEV, resulto;
		for (let o = 0; o < co; o++) is[o].lastUpdate = gIter;//Was: int
		for (let o = 0; o < co; o++) {//Was: int
			for (let a = 0; a < actionshere; a++) {//Was: int
...
//				let resulto = (player == 1 ? result1[o] : result2[o]); //Was: double
	...
	if (player == updatePlayer) {//«
		for (let o = 0; o < co; o++) {//Was: int
			gIss.put(infosetkeys[o], is[o], actionshere, 0); 
		}
	}//»
...
//	let maxNodesTouched = 0; //Was: ull
	...
	log("Reading the infosets from " , filename , "...");
	gIss.readFromDisk(filename);
	if (argc >= 3) maxIters = parseInt(argv[2]);
...
//Get the gIter from the name of the file
//		iter = parseInt(parts[1]); 
		gIter =	parseInt(parts[1]); //'gIter' is declared in Bluff_NS
	}
	gIter = Math.max(1,gIter);
	log("Set iteration to " , gIter);
...
//All of these const's were #define's«
...
// but I left it in case your game has abstraction, maybe this will be helpful
	...
	let bidseq = new Array(RECALL); //int bidseq[RECALL] = { 0 }; 
